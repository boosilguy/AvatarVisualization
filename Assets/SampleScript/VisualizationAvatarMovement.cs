...
public class VisualizationAvatarMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (prevBonesVectors[0] != null)
        {
            ...
            for(int i=0; i<bones.Length; i++)
            {
                ...
                Quaternion boneRotQuaternion = Quaternion.FromToRotation(prevBonesVectors[i], currentBoneVector);
                aBRPitch[i] = Mathf.Abs(Mathf.Rad2Deg * Mathf.Atan2(2 * boneRotQuaternion.x * boneRotQuaternion.w - 2 * boneRotQuaternion.y * boneRotQuaternion.z,  1 - 2 * boneRotQuaternion.x * boneRotQuaternion.x - 2 * boneRotQuaternion.z * boneRotQuaternion.z));
                aBRYaw[i] = Mathf.Abs(Mathf.Rad2Deg * Mathf.Atan2(2 * boneRotQuaternion.y * boneRotQuaternion.w - 2 * boneRotQuaternion.x * boneRotQuaternion.z, 1 - 2 * boneRotQuaternion.y * boneRotQuaternion.y - 2 * boneRotQuaternion.z * boneRotQuaternion.z));
                aBRRoll[i] = Mathf.Abs(Mathf.Rad2Deg * Mathf.Asin(2 * boneRotQuaternion.x * boneRotQuaternion.y + 2 * boneRotQuaternion.z * boneRotQuaternion.w));
    ...
    /// <summary>
    /// float 제네릭 타입 Queue의 평균 반환 함수 (Log 출력 전용 함수).
    /// </summary>
    /// <param name="q">평균을 구할 Queue.</param>
    /// <returns>
    /// 해당 메소드는 float 타입을 반환.
    /// </returns>
    float GetAvgValueInQueue(Queue<float> q, bool boolValue)
    {
        float sum = 0;
        foreach(float value in q)
        {
